...
// DA QUI SOTTO È TUTTO OLD E DA RIFARE
// SOPRA STO CONTINUANDO


		 
		// search for b/taus and find if they correspond to signal or noise (radiation)
		// use only last b (i.e. the one that will interact in the detector)
		if (
			 	IsLast(&(*p)) == true &&
			 ...
			 )
		{
	      ...
	      // save particles only if Higgs/b/tau/e/mu/nu (three flavours)
         if (
               (p->pdgId() == -25  && p->status() == 3) ||
               abs(p->pdgId()) == 15 ||
               abs(p->pdgId()) == 5 ||
...

         {
           ...
           // import particle 4vector and store in the TClonesArray
	        ...
	        // fill std::vectors with interesting data (id, mother, status)
	       gen_particle_ID_.push_back(p->pdgId());
      gen_particle_status_.push_back(p->status());
	   gen_particle_mother_.push_back(p->mother(0)->pdgId());

	const Candidate* cand_tau;

		if (abs(p->pdgId()) == 5 )
	{	
	   // for each tau, print all the "history" in terms of mother;
	   cand_tau = &(*p);
	   
	   cout << "\nThis is a b:\n";
	   while ( cand_tau->numberOfMothers() != 0)
	   {
	   	cout << "ID: " << cand_tau->pdgId() << " , mother: " << cand_tau->mother(0)->pdgId() << " , status: " << cand_tau->mother(0)->status() << endl;
	   	cand_tau =  cand_tau->mother(0);
		...
		// posso comprare gli indirizzi per vedere se la madre è la stessa
		if (p->pdgId() == -16 && (p->mother(0)== cand_tau) )
		{
			cout << "Mamma trovata!\n";
	   	cout << p->mother(0)->pdgId() << " " << cand_tau -> pdgId() << endl;
		}
	
		
		// print on screen all these data
	   cout << counterp-1 << " PdgId=" << p->pdgId() << " status=" << p->status() << " Mass=" << myvector.M()
	   ...
	   if(p->pdgId()==5)
	   cout << "-->";
	   cout <<counterp << " pdgID=" << p->pdgId() << " pt=" << p->pt() << endl;
...
// 	     << " costituents=" << (jets->getGenConstituents()).size()  << endl;
// */        
// 	nconstituents = (jets->getGenConstituents()).size();
// 	
// 	// import jet 4vector and store in the TCLonesArray
// 	setMomentum(myvector,jets->p4());
// 	new (gen_jet[counterj]) TLorentzVector(myvector);
// 	counterj++;
// 	
// 	// store num of constituents for each jet
// 	gen_jets_constituents_.push_back(nconstituents);
// 	       
//         njets++;
//        
...
//     }
//     
// //    cout << "Tot number of jets:  " << njets << endl;
// 
// 	 cout << "\n\n **ev ---------------------------------------- ev**\n\n\n";
    
    // fill the Ttree with the information stored
    
    // *************************************
    // WARNING:
    // WE ARE READING SIGNAL DATAFILE, SO THERE WILL ALWAYS BE NEW PARTICLES TO STORE FOR EACH EVENT
    // TO CHANGE IF SOME EVENTS WILL BE DISCARDED (ES. FOR BACKGROUND) OR THE Fill METHOD IN THIS POSITION WILL ALWAYS WRITE DATA (VOID DATA)
    // IN THIS CASE NEED TO ADD A "TRIGGER" FUNCTION THAT FILLS THE TREE ONLY IF THE EVENT IS GOOD
